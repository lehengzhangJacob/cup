...
def emotion_probs_from_logits(logits):
    ...
    if not (isinstance(logits, torch.Tensor) and logits.ndim == 2):
        raise ValueError(f"Expected logits as [batch, vocab], got {type(logits)} with shape {getattr(logits, 'shape', None)}")

    batch, vocab = logits.shape
    emo_keys = list(emotion_tokens.keys())
    token_ids = torch.tensor([emotion_tokens[k][0] for k in emo_keys], device=logits.device, dtype=torch.long)
    selected_logits = logits.index_select(dim=-1, index=token_ids)  # [batch, 3]

    emo_probs = torch.softmax(selected_logits, dim=-1)  # [batch, 3]

    results = []
    for b in range(batch):
        results.append({k: emo_probs[b, i].item() for i, k in enumerate(emo_keys)})
    return results
