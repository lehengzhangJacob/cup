...
def get_video_frames_count(video_path):
    """Robustly get video frame count using ffprobe with fallbacks.

    - Prefer ffprobe on the first video stream (v:0) to avoid multiple lines.
    - Try nb_read_frames, then nb_frames.
    - If parsing fails (e.g., output like "88\n1"), fall back to decord and OpenCV.
    """

    def _parse_first_int(text: str):
    ...
    ffprobe_cmds = [
        ...
        [
            ...
            '-show_entries', 'stream=nb_frames', '-of', 'default=nokey=1:noprint_wrappers=1', video_path
        ],
    ]
...
def process_video(video_path, processor, s=None, e=None, aspect_ratio='pad', num_frames=NUM_FRAMES):
    if isinstance(video_path, str):
        ...
        if os.path.isdir(video_path):                
            ...
            # Prefer ffprobe count; if unavailable, fall back to decord length.
            # To avoid mismatch, cap by decord length.
            decord_len = len(vreader)
            probed = get_video_frames_count(video_path)
            if not probed or probed <= 0:
                num_frames_of_video = decord_len
            else:
                num_frames_of_video = min(probed, decord_len)
        ...
        # Guard: clamp indices within valid bounds based on reader length
        if not os.path.isdir(video_path) and not video_path.endswith('.gif'):
            try:
                max_idx = len(vreader) - 1
                if max_idx >= 0:
                    sampled_frame_indices = np.clip(np.array(sampled_frame_indices, dtype=int), 0, max_idx)
        ...
        if os.path.isdir(video_path): 
            ...
            # Try batch read; fallback to per-frame on index errors
            try:
                ...
                for idx in sampled_frame_indices:
                    try:
                        arr = vreader[idx].asnumpy()
                        video_data.append(Image.fromarray(arr))
                    except Exception:
                        continue
    ...
    # Pad if not enough frames; ensure at least one frame exists before padding
    if num_frames is not None and len(video_data) > 0:
        while len(video_data) < num_frames:
            video_data.append(Image.fromarray(np.zeros((*video_data[-1].size, 3), dtype=np.uint8)))
    elif num_frames is not None and len(video_data) == 0:
        # As a last resort, create a single black frame of 224x224 to keep pipeline running
        dummy = Image.fromarray(np.zeros((224, 224, 3), dtype=np.uint8))
        video_data = [dummy for _ in range(num_frames)]
    if aspect_ratio == 'pad':
...
def process_video_qwen(video_path, processor, s=None, e=None, aspect_ratio='pad', num_frames=NUM_FRAMES):
    if isinstance(video_path, str):
        ...
        if os.path.isdir(video_path):                
            ...
            fps = vreader.get_avg_fps()
            # Prefer ffprobe count; if unavailable, fall back to decord length
            num_frames_of_video = get_video_frames_count(video_path)
            if not num_frames_of_video or num_frames_of_video <= 0:
...
def process_audio(audio_path, processor=None, sample_rate=16000, duration=10, s=None, e=None, return_empty=False):
    ...
    try:
        # 读取音频文件，并将其转换为 NumPy 数组，然后进一步转换为 PyTorch 张量
        ...
        # 检查音频数据中是否存在 NaN 值，如果存在，则用 -1.5 替换
        if torch.isnan(audio_data).any():
            audio_data = torch.nan_to_num(audio_data, nan=-1.5)
        ...
        # 如果音频数据是多通道的（例如立体声），则通过取平均值将其转换为单通道
