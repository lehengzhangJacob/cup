...
local_rank = None
bert_model = "/home/huggingface/bert-base-uncased"
...
os.environ["NCCL_DEBUG"] = "DEBUG"  # 可选，调试时使用
# 设置超时时间为3分钟（180秒）
os.environ["NCCL_TIMEOUT"] = "10"
...
class DepressionDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        self.depression_datasets = ['AVEC2014', 'CMDC', 'DAIC-WOZ', 'E-DAIC', 'EATD', 'modma']
        self.emotion_datasets = ['meld', 'IEMOCAP']
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "video" in sample:
            ...
            try:
                video = process_video(video_path, video_processor, 
                                    aspect_ratio=self.data_args.image_aspect_ratio, 
                                    num_frames=num_frames)
                videos.append(video)
            except Exception as e:
                backup_idx = random.randint(0, len(self.list_data_dict) - 1)
                print(f"Encounted error({e}) when reading video {video_file}, use {backup_idx}-th example instead!!!")
                return self.__getitem__(backup_idx)

        # Process audios
        audios = []
        if "audio" in sample and audio_processor is not None:
            audio_folder = getattr(self.data_args, 'audio_folder', self.data_args.data_folder)
            ...
            try:
                import torchaudio
                audio_data, sample_rate = torchaudio.load(audio_path, normalize=True, channels_first=True)
                audio_data = torchaudio.transforms.Resample(orig_freq=sample_rate, 
                                                            new_freq=audio_processor.sampling_rate)(audio_data)
                if len(audio_data.shape) > 1:
                    audio_data = audio_data.mean(dim=0)
                audio_features = audio_processor(audio_data, sampling_rate=audio_processor.sampling_rate, 
                                                return_tensors='pt')['input_features']
                audios.append(audio_features)
            except Exception as e:
                rank0_print(f"Error processing audio {audio_path}: {e}")
                if audio_processor:
                    zero_audio = torch.zeros(1, audio_processor.feature_size, audio_processor.nb_max_frames)
                    audios.append(zero_audio)            

        # Process images 
        images = []
        if "image" in sample:
            image_folder = getattr(self.data_args, 'image_folder', self.data_args.data_folder)
            ...
            try:
                image = process_image(image_path, image_processor, 
                                    aspect_ratio=self.data_args.image_aspect_ratio)
                images.append(image)
            except Exception as e:
                rank0_print(f"Error processing image {image_path}: {e}")
                backup_idx = random.randint(0, len(self.list_data_dict) - 1)
                return self.__getitem__(backup_idx)

        # Preprocess conversations with multi-modal tokens
        ...
        if videos:
            data_dict['video'] = videos[0]
        if audios:
            data_dict['audio'] = audios[0]  
        if images:
...
@dataclass
class DataCollatorForDepressionDataset(object):
    """Collate examples for depression training."""
...
def make_depression_data_module(tokenizer: transformers.PreTrainedTokenizer,
                                data_args) -> Dict:
    """Make dataset and collator for depression fine-tuning."""
    train_dataset = DepressionDataset(
        tokenizer=tokenizer,
        data_path=data_args.data_path,
        data_args=data_args
    )
    data_collator = DataCollatorForDepressionDataset(tokenizer=tokenizer)
    return dict(train_dataset=train_dataset,
                eval_dataset=None,
                data_collator=data_collator)
...
def train(attn_implementation=None):
   ...
   # import ipdb; ipdb.set_trace()
    ...
    # 第一阶段训练：投影器、编码器
    model.requires_grad_(False)
    tune_modules = ['mm_projector', 'pos_emb', 'audio_projector', 'vision_sampler', 'vision_tower', 'audio_tower']
    for name, param in model.named_parameters():
        if any(listed_name in name for listed_name in tune_modules):
            # rank0_print('tuning {}'.format(name))
            param.requires_grad = True
    ...
    data_module = make_depression_data_module(tokenizer=tokenizer, data_args=data_args)
