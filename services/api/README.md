...
首屏会立即并行建立 WebRTC，Live2D 备用引擎及约 3MB 资源仅在真人数字人失败时加载。云端问答默认使用 `glm-4.5-flash`，首 token 超过 2 秒会切换 `glm-4-flash-250414`。语音只在句末标点或足够长的自然分句处切分，不再按 12 字硬切；120 字无标点才启用安全切分。2026-07-19 当前质量优先配置的本机热链路三次中位数为：首 token 535ms、首响 3618ms、最后音频 10773ms。用户正常阅读或录音期间会并行完成数字人连接。
...
情绪基座。`EMOTION_PYTHON` 必须能导入项目内置的 `humanomni`
（至少包括 `model_init`、`mm_infer`）、`peft` 和 `torch`。训练代码若另外提供
`emotion_probs_from_logits`，适配器会保存七分类概率；官方生成式实现只返回标签时，
置信度为 0，不会伪造概率。
