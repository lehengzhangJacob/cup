...
class RAGClientTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_lightweight_local_route_is_forwarded(self):
        async def handler(request: httpx.Request) -> httpx.Response:
            payload = json.loads(request.content)
            self.assertEqual(payload["model_route"], "local_lite")
            return httpx.Response(
                200,
                json={"session_id": "session-1", "answer": "轻量回答"},
            )
        ...
        result = await client.chat(
            "问题",
            session_id="session-1",
            model_route="local_lite",
        )
        self.assertEqual(result["answer"], "轻量回答")

    async def test_non_streaming_error_preserves_server_detail(self):
        async def handler(_: httpx.Request) -> httpx.Response:
            return httpx.Response(
                502,
                json={"detail": "轻量本地模型发生 CUDA 显存不足（OOM）"},
            )
        ...
        with self.assertRaisesRegex(RAGClientError, "OOM"):
            await client.chat("问题", session_id="session-1")
